...
public class DebugCameraConfig {
    ...
    private static final float DEFAULT_MAX_ZOOM_OUT = 20.0f;
    ...
    private void init() {
        ...
        if (fileHandle.exists()) load();
        else {
            log.info(FILE_PATH + " File not found");
            setDefaults();
        }
    }
    ...
    private void load() {
        try {
            ...
            log.error("Error loading " + FILE_PATH + ", using default settings...");
            setDefaults();
        }
    }
